...
function PlacesForm({ handleClose }: Props) {
  ...
  useEffect(() => {
    if(name && place.imagesId && place.imagesId.length > 0 && selectedCategories
      && selectedCategories.length > 0 && address
      && address.route && address.latitude && address.longitude) {
        setIsFormValid(true)
      } else {
        setIsFormValid(false)
      }
  }, [name, place.imagesId, selectedCategories, address, address.route, address.latitude, address.longitude])
  ...
  return (
    <div className="flex">
      ...
      >
        ...
        {
          addressError && <label className="my-1 text-red-500">{addressError}</label>
        }
        ...
        {
          imagesError && <label className="my-1 text-red-500">{imagesError}</label>
        }

        {images && images.length > 0 && (
          <div className="flex flex-row flex-wrap gap-2">
            {images.map((image) => (
              <div key={image.id} className="relative max-w-[146px] text-center bg-white shadow flex flex-col justify-center items-center px-4 py-2 rounded-md">
                <label
                onClick={() => setImages(images.filter(i => i.id !== image.id))}
                className="hover:bg-gray-600 hover:shadow-md transition-colors duration-200 flex justify-center items-center rounded-full bg-black w-[24px] h-[24px] cursor-pointer text-white absolute right-2 top-2"><FaXmark/></label>
                <label className="pb-6 pt-2 text-4xl"><AiFillFileImage /></label>
        ...
        {
          categoriesError && <label className="my-1 text-red-500">{categoriesError}</label>
        }
        ...
        <input
          ...
          className={`${!isFormValid ? 'cursor-default opacity-40' : 'cursor-pointer hover:text-black hover:bg-white'} transition-color duration-300 px-4 py-2 border-2 border-white text-white bg-primary`}
          type="submit"
          value="Save"
        />
      </form>
