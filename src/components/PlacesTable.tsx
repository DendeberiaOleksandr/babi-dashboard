...
function PlacesTable({ places, setSelectedPlace }: Props) {
  ...
  return (
    <div className="w-full h-full">
      <table className={`overflow-x-auto overflow-y-auto block w-full border-collapse border-2 border-primary`}>
        ...
        <tbody className={borderClass}>
          {places &&
            places.map((place) => (
              <tr
                className={`${borderClass} hover:bg-gray-300 duration-200 transition-colors cursor-pointer`}
                key={place.id}
              >
                <td
                  onClick={(e) => handlePlaceClick(place)}
                ...
                <td className={borderClass}>
                  {place.categories && place.categories.length > 0 && (
                    <select
                      ...
                      defaultValue={place.categories[0].id}
                    >
                ...
                <td
                  ...
                  className={borderClass}
                >
                  {place.address?.administrativeAreaLevel1}
                </td>
                <td
                  onClick={(e) => handlePlaceClick(place)}
                  className={borderClass}
                >
                  {place.address?.administrativeAreaLevel2}
                </td>
                <td
                  onClick={(e) => handlePlaceClick(place)}
                  className={borderClass}
